...
def build_gru_model(input_shape):
    model = Sequential()
    # GRU layer with 64 units, input shape is (timesteps, features)
...
X_train_padded.shape #no. of data, no. of freq bins, time steps



# Model summary
input_shape = (X_train_padded.shape[2], X_train_padded.shape[1]) #input_shape=(timesteps, features)
...
# Train the model
gru_model.fit(X_train_padded, y_train, epochs=30, batch_size=16, validation_data= (X_val_padded, y_val),class_weight=class_weights, callbacks=[early_stopping])


# ## 3.2 Testing


gru_y_pred_proba = gru_model.predict(X_test_padded).flatten()
...
gru_auc = roc_auc_score(y_test, gru_y_pred_proba)
gru_eer = calculate_eer(y_test, gru_y_pred_proba)
...
print(f"  - AUC: {gru_auc:.4f}")
print(f"  - EER: {gru_eer:.4f}")
