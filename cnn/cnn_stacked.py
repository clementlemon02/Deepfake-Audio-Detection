...
# Configure logging
logging.basicConfig(filename='cnn_stacked_training_log.txt', level=logging.INFO, format='%(asctime)s - %(message)s')
...
class StackedSpectrogramDataset(Dataset):
    def __init__(self, data_dir, label_lookup, transform=None):
        self.data_dir = data_dir
        self.transform = transform
        self.label_lookup = label_lookup
        self.file_list = [f for f in os.listdir(data_dir) if f.endswith('_stacked.npy')]
    ...
    def __getitem__(self, idx):
        ...
        base_name = file_name.split('_')[0]  # Extract base file name (without '_stacked.npy')
        file_path = os.path.join(self.data_dir, file_name)

        # Load stacked spectrogram
        ...
        # Get label based on file ID
        label = self.label_lookup.get(base_name, 0)
...
class AudioCNN(nn.Module):
    def __init__(self, num_classes=1):
        ...
        # Define fc1 with a temporary input size; will be corrected after forward pass
        self.fc1 = None
    ...
    def forward(self, x):
        ...
        # Compute the flattened size dynamically if not already set
        if self.fc1 is None:
...
train_dir = './output/stacked'
val_dir = './output/stacked'
test_dir = './output/stacked'

# Initialize datasets and DataLoaders
train_dataset = StackedSpectrogramDataset(train_dir, label_lookup)
val_dataset = StackedSpectrogramDataset(val_dir, label_lookup)
test_dataset = StackedSpectrogramDataset(test_dir, label_lookup)
...
print("Model saved as 'audio_stacked_cnn_model.pth'")
